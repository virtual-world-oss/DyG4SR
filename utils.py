...
def get_ddygs_CDs(ratings_path, num_shots):
    with open(ratings_path, 'rb') as f:
        ratings = pkl.load(f)
    times = ratings['timestamp'].tolist()
    MAX_TIME = max(times)
...
def get_ddygs_CDs_dense(ratings_path, num_shots):
    with open(ratings_path, 'rb') as f:
        ratings = pkl.load(f)
    times = ratings['timestamp'].tolist()
    MAX_TIME = max(times)
...
if __name__ == '__main__':
    ...
    ddygs, time_list = get_ddygs_CDs('data/CDs/ratings.pkl', num_shots=30)
    ddygs, time_list = get_ddygs_CDs_dense('data/CDs/ratings.pkl', num_shots=30)

    for i, ddyg in enumerate(ddygs):
      print(i, ddyg.shape)
    for i, time in enumerate(time_list):
      print(i, time)
    # with open('data/CDs/ddygs.pkl', 'wb') as f:
    #     pkl.dump(ddygs, f)